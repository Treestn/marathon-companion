...
const getActiveObjectives = (quest: Quest): ActiveObjective[] => {
  const activeObjectives: ActiveObjective[] = [];
...
export const SubscribedSidePanel: React.FC<SubscribedSidePanel> = ({
  ...
  const questObjectives = useMemo(() => {
    const map = new Map<string, ActiveObjective[]>();
    quests.forEach((quest) => {
      map.set(quest.id, getActiveObjectives(quest));
    });
    return map;
  }, [quests]);
        ...
        {quests.length === 0 ? (
          ...
          <div className="subscribed-quests-list scroll-div">
            {orderedQuests.map((quest) => {
              const objectives = questObjectives.get(quest.id) ?? [];
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span
                      className={`subscribed-quest-chevron${
                        isOpen ? ' subscribed-quest-chevron-open' : ''
                      }`}
                    >
                      {isOpen ? '▾' : '▸'}
                    </span>
