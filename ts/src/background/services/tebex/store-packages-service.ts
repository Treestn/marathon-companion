...
> {
  ...
  private currentlySelectedPackage: StorePackage = null;
  
  public GetCurrentPackages() {
    return this.currentPackages;
  }
  
  public getPackageFromId(id:string):StorePackage {
    for(let i = 0; this.currentPackages.length; i++) {
      if(this.currentPackages[i].id.toString() === id) {
        return this.currentPackages[i]
      }
    }
  }
  
  public setCurrentlySelectedPackage(storePackage: StorePackage) {
    this.currentlySelectedPackage = storePackage;
  }

  public getCurrentlySelectedPackage(): StorePackage {
    return this.currentlySelectedPackage;
  }
  ...
  public constructor(
    ...
    this.listView = renderListService.CreateRenderer<StorePackage>((pack) => {
      ...
      console.log(document.getElementById("subscription-card"));
